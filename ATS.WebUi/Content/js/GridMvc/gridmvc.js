...
GridMvc = (function ($) {
    ...
    gridMvc.prototype.init = function () {
        ...
        //this.addFilterWidget(new StatusFilterWidget());
        this.addFilterWidget(new VacancyFilterWidget());
...
NumberFilterWidget = (function ($) {
    ...
    numberFilterWidget.prototype.renderWidget = function () {
        var html = '<div class="form-group">\
                        ...
                        <select class="grid-filter-type form-control">\
                            <option value="1" ' + (this.value.filterType == "1" ? "selected=\"selected\"" : "") + '>' + this.lang.filterSelectTypes.Equals + '</option>\
                            <option value="5" ' + (this.value.filterType == "5" ? "selected=\"selected\"" : "") + '>' + this.lang.filterSelectTypes.Greater_Than + '</option>\
                            <option value="6" ' + (this.value.filterType == "6" ? "selected=\"selected\"" : "") + '>' + this.lang.filterSelectTypes.Less_Than + '</option>\
                        </select>\
                    </div>\
                    <div class="form-group">\
                        <label>' + this.lang.filterValueLabel + '</label>\
        ...
        if (this.datePickerIncluded) {
            ...
            dateContainer.datepicker({
                onSelect: function (dateText) {
                    ...
                    var type = $context.container.find(".grid-filter-type").val();
                    //grid-filter-col=User.BirthDate&grid-filter-type=1&grid-filter-val=30-10-2013
                    //grid-filter-col=AppliedOn&grid-filter-type=1&grid-filter-val=08/04/2014
                    if (type == "1") {
            ...
            //dateContainer.datepicker(datePickerOptions).on('onSelect', function (dateText,ev) {
            //    var type = $context.container.find(".grid-filter-type").val();
...
function StatusFilterWidget() {
    ...
    */
    this.onRender = function (container, lang, typeName, values, cb, data) {
        ...
        this.value = values.length > 0 ? values[0] : { filterType: 11, filterValue: "" };

        this.renderWidget(); //onRender filter widget
        this.loadStatus(); //load customer's list from the server
        this.registerEvents(); //handle events
    };
    this.renderWidget = function () {
        var str = this.value.filterValue;
        var html = '<input id="selectAll" type="checkbox" class="grid-filter-input form-control" value="Select All">Select All</option><br>'
            + '<div style="width:auto;" class="statuslist">\
                    </div>'+ '<div class="grid-filter-buttons">\
                                <input type="button" class="btn btn-primary grid-apply" value="' + this.lang.applyFilterButtonText + '" />\
                             </div>';
        this.container.append(html);
    };
    /***
    * Method loads all Status from the server via Ajax:
    */
    this.loadStatus = function () {
        var $this = this;
        //$.post("/Employee/RndType/GetStatusList", function (data) {
        //    $this.fillStatus(data);
        //});
        $.ajax({
            type: 'POST',
            url: '/Employee/RndType/GetStatusList',
            data: this.value.filterValue,
            success: filldata,
            async: false
        });
    ...
    /***
    * Method fill Status select list by data
    */
    this.fillStatus = function (items) {
        var statusList = this.container.find(".statuslist");
        if (setStatus(this.value.filterValue, 'Draft'))
            statusList.append('<input checked onchange="selectone(this);" type="checkbox" class="grid-filter-input form-control chk" ' + (' value="' + 'Draft' + '">' + 'Draft' + '</option><br>'));
        else
            statusList.append('<input onchange="selectone(this);" type="checkbox" class="grid-filter-input form-control chk" ' + (' value="' + 'Draft' + '">' + 'Draft' + '</option><br>'));

        for (var i = 0; i < items.length; i++) {
            var isChecked = setStatus(this.value.filterValue, items[i].Text);
            if (isChecked)
                statusList.append('<input checked onchange="selectone(this);" type="checkbox" class="grid-filter-input form-control chk" ' + ' value="' + items[i].Text + '">' + items[i].Text + '</option><br>');
            else
                statusList.append('<input onchange="selectone(this);" type="checkbox" class="grid-filter-input form-control chk" ' + ' value="' + items[i].Text + '">' + items[i].Text + '</option><br>');
        }
        if (setStatus(this.value.filterValue, 'Decline'))
            statusList.append('<input checked onchange="selectone(this);" type="checkbox" class="grid-filter-input form-control chk" ' + (' value="' + 'Decline' + '">' + 'Decline' + '</option><br>'));
        else
            statusList.append('<input onchange="selectone(this);" type="checkbox" class="grid-filter-input form-control chk" ' + (' value="' + 'Decline' + '">' + 'Decline' + '</option><br>'));

        function setStatus(filterString, text) {
            if (filterString != undefined && filterString != "") {
                var arraylist = filterString.split('~');
    ...
    */
    this.registerEvents = function () {
        ...
        applyBtn.click(function () {
            ...
            var type = "CustomFilter";//$context.container.find(".grid-filter-type").val();
            var strvalue = "";//$context.container.find(".grid-filter-input").val();
            $(".chk").each(function () {
                if (this.checked) {
                    if (strvalue == "")
                        strvalue = this.value;
            ...
            var filterValues = [{ filterType: type, filterValue: strvalue }];
            $context.cb(filterValues);
        });
        this.container.find(".grid-filter-input").keyup(function (event) {
            if (event.keyCode == 13) {
...
function VacancyFilterWidget() {
    /***
    * This method must return type of registered widget type in 'SetFilterWidgetType' method
    */

    this.getAssociatedTypes = function () {
        return ["CustomVacancyFilterWidget"];
    };
    /***
    * This method invokes when filter widget was shown on the page
    */
    ...
    */
    ...
    this.renderWidget = function () {
        var str = this.value.filterValue;
        var html = '<div style="width:200px; max-height:120px; overflow-y:scroll; font-size: 11px;"><input id="selectAll" type="checkbox" class="grid-filter-input form-control" value="Select All">Select All</option><br>'
            + '<div style="width:auto;" class="vacancyList">\
                    </div>'+ '</div><div class="grid-filter-buttons">\
                                <input type="button" class="btn btn-primary grid-apply" value="' + this.lang.applyFilterButtonText + '" />\
                             </div>';
        this.container.append(html);
    };
    ...
    */
    this.loadStatus = function () {
        ...
        var _status=getParameterByName('Status');
        $.ajax({
            type: 'POST',
            url: '/Employee/MyVacancy/GetVacancyListForFilter',
            data: { applicationStatus: _status },
            success: filldata,
            async: false
        });
        function filldata(data) {
    ...
    */
    this.fillStatus = function (items) {
        var statusList = this.container.find(".vacancyList");
        //if (setStatus(this.value.filterValue, 'Draft'))
        //    statusList.append('<input checked onchange="selectone(this);" type="checkbox" class="grid-filter-input form-control chk" ' + (' value="' + 'Draft' + '">' + 'Draft' + '</option><br>'));
        //else
        //    statusList.append('<input onchange="selectone(this);" type="checkbox" class="grid-filter-input form-control chk" ' + (' value="' + 'Draft' + '">' + 'Draft' + '</option><br>'));
        ...
        for (var i = 0; i < items.length; i++) {
            var isChecked = setStatus(this.value.filterValue, items[i].Value);
            if (isChecked)
                statusList.append('<input checked onchange="selectone(this);" type="checkbox" class="grid-filter-input form-control chk" ' + ' value="' + items[i].Value + '">' + items[i].Text + '</option><br>');
            else
                statusList.append('<input onchange="selectone(this);" type="checkbox" class="grid-filter-input form-control chk" ' + ' value="' + items[i].Value + '">' + items[i].Text + '</option><br>');
        }
        //if (setStatus(this.value.filterValue, 'Decline'))
        //    statusList.append('<input checked onchange="selectone(this);" type="checkbox" class="grid-filter-input form-control chk" ' + (' value="' + 'Decline' + '">' + 'Decline' + '</option><br>'));
        //else
    ...
    */
    ...
    function getParameterByName(name) {
        ...
        return results === null ? "Active" : decodeURIComponent(results[1].replace(/\+/g, " "));
    }
}